...
def word_frequency(lyrics_file, word):
    ...
    if lyrics:
        ...
        songs = lyrics.split("###")[
            1:
        ]  # Skip the first split part, which is the header
        song_counts = []
        total_count = 0
        songs_with_word = 0
        ...
        percentage_songs_with_word = (
            (songs_with_word / len(song_counts) * 100) if song_counts else 0
        )
        counts = [count for _, count in song_counts]
        std_deviation = (
            calculate_standard_deviation(counts, average_count) if song_counts else 0
        )
        ...
        if most_frequent_song:
            print(
                f"The song with the most occurences has {most_frequent_song[1]} occurrences."
            )
        else:
            print("No song has the most occurrences.")
        if least_frequent_song:
            print(
                f"The song with the least occurrences has {least_frequent_song[1]} occurrences."
            )
        else:
            print("No song qualifies for the least occurrences.")
        print(
            f"Percentage of songs containing the word: {percentage_songs_with_word:.2f}%"
        )
